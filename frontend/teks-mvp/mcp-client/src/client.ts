...
import { openDb, upsertRun, insertSteps, StepRow } from "./db.js";
...
// ---------- Types ----------
type StepAction = "navigate" | "type" | "click" | "waitFor" | "screenshot" | "assertVisible";
type Step =
...
function writeTXT(p: string, text: string) { fs.writeFileSync(p, text); }
function safePath(base: string, name: string) { return path.join(base, name); }
function redact(s?: string) { return s ? (s.length <= 12 ? "***" : s.slice(0, 6) + "..." + s.slice(-4)) : s; }
function cleanEnv(src: NodeJS.ProcessEnv): Record<string, string> {
  return Object.fromEntries(Object.entries(src).filter(([, v]) => v !== undefined) as [string, string][]);
}
...
const allowed: StepAction[] = ["navigate", "type", "click", "waitFor", "screenshot", "assertVisible"];

function validateSteps(steps: any[]) {
  if (!Array.isArray(steps)) return { ok: false as const, error: "Output is not an array" };
  for (let i = 0; i < steps.length; i++) {
    const s = steps[i];
    if (!s || typeof s !== "object") return { ok: false as const, error: `Step ${i + 1} is not an object` };
    if (!allowed.includes(s.action)) return { ok: false as const, error: `Step ${i + 1}: invalid action '${s.action}'` };
    if (["type", "click", "waitFor", "assertVisible"].includes(s.action) && typeof s.selector !== "string")
      return { ok: false as const, error: `Step ${i + 1}: '${s.action}' requires 'selector'` };
    if (s.action === "type" && typeof s.value !== "string")
      return { ok: false as const, error: `Step ${i + 1}: 'type' requires 'value'` };
    if (s.action === "screenshot" && s.name && typeof s.name !== "string")
      return { ok: false as const, error: `Step ${i + 1}: screenshot 'name' must be string` };
  }
  return { ok: true as const, steps: steps as Step[] };
}

// ----- Normalization: accept shorthand like { "navigate": "/x" } -----
type AnyObj = Record<string, any>;

// small helpers to derive selectors from "field" / "text"
function selectorFromField(field: string): string {
  // Prefer label-based targeting (works with Playwright selectors the MCP understands)
  return `label=${field}`;
}
function selectorFromTextForClick(text: string): string {
  // Try a role button first; runner can fall back in prelude or later steps if needed
  return `role=button[name="${text}"]`;
}
function selectorFromText(text: string): string {
  // Generic "element containing text"
  return `text=${text}`;
}

function normalizeStep(obj: AnyObj): Step | null {
...
if (obj.action === "waitFor" && typeof obj.urlContains === "string" && !obj.selector) {
  ...
  if (typeof obj.action === "string") {
    ...
    if ((obj.action === "waitFor" || obj.action === "assertVisible") &&
      ...
      return { action: obj.action, selector: selectorFromText(obj.text) } as Step;
    }
  ...
  if (obj.type && typeof obj.type === "object") {
    ...
    if (sel && val) return { action: "type", selector: sel, value: val };
  }
  // { "type": "selector=...,value=..." } (rare)
  if (typeof obj.type === "string") {
    const m = obj.type.match(/selector=(.*?),\s*value=(.*)/);
    if (m) return { action: "type", selector: m[1].trim(), value: m[2].trim() };
  }
...
async function planWithNovaLite(plain: string, outDir: string): Promise<Step[]> {
const system = `
...
`.trim();
  ...
  try {
    ...
    const text: string = json?.output?.message?.content?.[0]?.text ?? "";
...
// ---------- MCP wiring ----------
async function withPlaywrightMCP<T>(fn: (client: Client, screenshotToolName: string | null) => Promise<T>): Promise<T> {
  ...
  try { return await fn(client, screenshotToolName); }
  finally { await client.close(); }
}

async function callToolWithTimeout(client: Client, name: string, args: Record<string, unknown>, timeoutMs: number) {
...
async function apiCreateStudent(authToken: string): Promise<{ id: string, localId: string, name: string }> {
  ...
  if (!r.ok) throw new Error(`createStudent failed: HTTP ${r.status}`);
  const j = await r.json().catch(() => ({}));
...
async function executeSteps(
  ...
  steps: Step[],
  outDir: string,
  screenshotToolName: string | null,
  visitedUrls: string[] = []
) {
  const results: RunEntry[] = [];

  async function tryScreenshot(label: string, idx: number) {
  ...
  let currentUrlTool: string | null = null;
  try {
    const tools = await client.listTools();
    currentUrlTool = tools.tools?.find(t => /current.*url|get.*url/i.test(t.name))?.name ?? null;
  } catch { }
  async function recordUrlFallback(url?: string) {
    if (url) { visitedUrls.push(url); return; }
    if (!currentUrlTool) return;
    try {
      const res: any = await callToolWithTimeout(client, currentUrlTool, {}, 2000);
      const u = (res as any)?.content?.[0]?.text || (res as any)?.url || "";
      if (u) visitedUrls.push(String(u));
    } catch { }
  }

  for (const [i, step] of steps.entries()) {
    const started = Date.now();
    const entry: RunEntry = { ...(step as any), index: i + 1, status: "passed", durationMs: 0 };

    try {
      switch (step.action) {
        case "navigate": {
          const url = toAbsoluteUrl((step as any).url || "/");
        ...
        case "type": {
          const s = step as Extract<Step, { action: "type" }>;
        ...
        case "waitFor": {
          const s = step as Extract<Step, { action: "waitFor" }>;
        ...
        case "screenshot": {
          const s = step as Extract<Step, { action: "screenshot"; name?: string }>;
        ...
        case "assertVisible": {
          const s = step as Extract<Step, { action: "assertVisible" }>;
          await callToolWithTimeout(client, "browser_wait_for", { element: s.selector }, STEP_TIMEOUT_MS);
          await recordUrlFallback();
...
function renderHtml(opts: {
  ...
  <table>
    ...
    ${steps.map(s => `
      <tr>
        <td>${s.index}</td>
        <td>${(s as any).action}</td>
        <td class="${s.status === 'passed' ? 'ok' : 'fail'}">${s.status}</td>
        <td>${s.durationMs}</td>
        <td>${(s as any).selector || (s as any).url || ''}</td>
        <td>${s.error ?? ''}</td>
...
async function main() {
  ...
  try {
    ...
    await withPlaywrightMCP(async (client, shotName) => {
      ...
      const stepRows: StepRow[] = results.map(r => ({
        run_id: RUN_ID,
        idx: r.index,
        action: (r as any).action,
        selector: (r as any).selector ?? null,
